...
- https://build123d.readthedocs.io/en/latest/_sources/tutorial_constraints.rst.txt

Use this page for how build123d uses precise construction and targeted constraint helpers instead of a broad sketch constraint solver.
## Introduction
...
`BlendCurve`, `ConstrainedLines`,
`ConstrainedArcs`, and `Triangle`.
It also provides a growing family of constructors whose extent can be determined by other
geometry, including `PolarLine`, `CenterArc`,
`EllipticalCenterArc`, `EllipticalStartArc`,
`JernArc`, `ParabolicCenterArc`, and
`HyperbolicCenterArc`. Together with operations such as
`make_hull`, `mirror`, and
`offset`, these tools solve specific constraint patterns while
...
`PolarLine`, `CenterArc`, `EllipticalCenterArc`, `EllipticalStartArc`, `JernArc`, `ParabolicCenterArc`, and `HyperbolicCenterArc`
   ...
   - a limiting object such as a `Shape`, `Axis`,
     `Location`, `Plane`, or point-like object.
...
build123d includes a built-in `Triangle` object that has an internal solver such that one can
...
`PolarLine` creates each line from a start point and direction,
...
`offset` preserves the source profile shape while enforcing constant wall thickness.
This is a common pattern for clearances, shells, and manufacturing margins.

## Tangency Constraints

Both `ConstrainedArcs` and `ConstrainedLines`
return a `Curve` containing one or more `Edge` objects.
...
[ocp-vscode](https://github.com/bernhard-42/vscode-ocp-cad-viewer) viewer is very handy as
...
4. Use constrained constructors only where they add value (for example `ConstrainedArcs`),
   and use direct primitives elsewhere.
5. Create a `Face` (`make_face` then center-hole subtraction) only after
