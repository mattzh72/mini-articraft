...
- https://build123d.readthedocs.io/en/latest/_sources/tips.rst.txt

Use this page for build123d modeling tips, best practices, FAQ guidance, and common pitfalls.
...
As discussed in the Assembly section, a `shallow copy` of parts that
...
As both [CadQuery](https://cadquery.readthedocs.io/en/latest/index.html) and **build123d** use
a common OpenCascade Python wrapper ([OCP](https://github.com/CadQuery/OCP)) it's possible to
...
once. `pack` will translate the `Shape`'s passed to it so
...
build123d is most commonly used not as a library within a larger application, but as a [Domain-Specific Language](https://en.wikipedia.org/wiki/Domain-specific_language)_
...
example of sketching on a 3D part, see `sketching_on_other_planes`.
