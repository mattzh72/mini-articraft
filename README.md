...
A simpler, sharper articraft. Give it a prompt, get back an articulated 3D object: an
agent writes and iterates on a build script against the mesh SDK, the compiler checks
the geometry (including the articulation in motion), and the result exports as a
posable USDZ you can open in the built-in viewer.
